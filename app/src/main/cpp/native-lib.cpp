...
Java_com_jchen_playerj_PlayerJ_native_1prepare(JNIEnv *env, jobject thiz, jstring url) {
    //转化播放源地址.
...
extern "C"
JNIEXPORT void JNICALL
Java_com_jchen_playerj_PlayerJ_native_1start(JNIEnv *env, jobject thiz) {
    if (jFFmpeg) {
        jFFmpeg->start();
    }
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_jchen_playerj_PlayerJ_native_1set_1surface(JNIEnv *env, jobject thiz, jobject surface) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_jchen_playerj_PlayerJ_native_1pause(JNIEnv *env, jobject thiz) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_jchen_playerj_PlayerJ_native_1close(JNIEnv *env, jobject thiz) {
    //1. 停止video解码
    if (jFFmpeg) {
        jFFmpeg->close();
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_jchen_playerj_PlayerJ_native_1seek(JNIEnv *env, jobject thiz, jlong ms) {
    if(jFFmpeg){
        jFFmpeg->seek(ms);
    }
}